...
class TestOverrotation(unittest.TestCase):
    ...
    def disabled_test_directional_convergence_summing(self):
        ...
        self.assertTrue(
            np.dot(reference_vector, convergence_vector)
    ...
    def plottest_single_ellipse_radius(
        ...
        InitialDynamics = LinearSystem(
            attractor_position=np.array([5.5, -4.5]),
        ...
        ConverginSystem = LinearSystem(
            attractor_position=np.array([5.5, -4.5]), maximum_velocity=0.5
        )
        ...
        for it_obs, obs in zip(range(len(obstacle_list)), obstacle_list):
            for it in range(n_resolution * n_resolution):
                ...
                if assert_check and it >= 1:
                    ...
                    if not np.isclose(conv_radius[it], conv_radius[it - 1]):
                        ...
                        self.assertEqual(
                            conv_radius[it] > conv_radius[it - 1],
                            gamma[it] < gamma[it - 1],
            ...
            if visualize:
                ...
                ax.quiver(
                    positions[0, :],
                    positions[1, :],
                    rotated_velocities[0, :],
                    rotated_velocities[1, :],
                    color="black",
                    zorder=3,
                )
    ...
    def plottest_two_ellipse_radius(
        self, assert_check=True, visualize=False, save_figure=False
    ):
        """Cretion & adapation of MultiWall-Surrounding"""
        dim = 2
