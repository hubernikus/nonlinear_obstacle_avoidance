...
def visualize_displacements(
    ...
    for it_nonl, inv_nonlinear in enumerate(inv_nonlinear_list):
        for it_conv, inv_conv_rotated in enumerate(inv_conv_rotated_list):
            ...
            plt.plot(
                [inv_nonlinear._angle[0], inv_conv_rotated._angle[0]],
            ...
            plt.plot(
                inv_conv_rotated._angle[0], inv_conv_rotated._angle[1], "o", color="b"
            )
            plt.plot(
                inv_conv_proj._angle[0],
                inv_conv_proj._angle[1],
                "+",
                color="b",
                markersize=8,
                markeredgewidth=3,
            ...
            plt.plot(
                inv_nonlinear._angle[0],
                inv_nonlinear._angle[1],
                "o",
                color="r",
            )
            plt.plot(
                inv_nonl_rotated._angle[0],
                inv_nonl_rotated._angle[1],
                "x",
                color="r",
                markersize=8,
                markeredgewidth=3,
    ...
    circular_space_setup(
        ax,
        circle_background_color="#bee8c2",
    ...
    if save_figure:
        ...
        plt.savefig(
            "figures/" + figure_name + radius_name + ".png", bbox_inches="tight"
        )
...
class TestProjectionOfDisplacement(unittest.TestCase):
    ...
    def test_radius_pi_quarter(self):
        ...
        self.assertTrue(
            np.allclose(inv_conv_proj.as_angle(), inv_conv_rotated.as_angle())
        )
...
if (__name__) == "__main__":
    ...
    if visualize:
        ...
        radius_name_list = [
            "_radius_of_pi_quarter",
            "_radius_of_pi_half" "_radius_of_three_pi_quarter",
        ]
