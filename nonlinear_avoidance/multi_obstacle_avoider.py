...
class MultiObstacleAvoider:
    ...
    def compute_convergence_sequence(
        ...
        # How does it behave with highly concave obstacles
        ...
        # for node in self.conv_tree._graph.nodes():
        #     breakpoint()
        #     self.conv_tree._graph.nodes[node]
        #     #     if np.any(np.isnan(self.conv_tree._graph.nodes[node]["orientation"].base)):
    ...
    def compute_weights_from_distances(
        distances: np.ndarray, distance_min: float = 1.0
    ) -> np.ndarray:
        ...
        if (weights_sum := np.sum(weights)) > 1.0:
